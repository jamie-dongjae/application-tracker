...
- **Insights** — an [Apache ECharts](https://echarts.apache.org)-powered dashboard: a pipeline-flow sankey (history-aware — an application that reached Interview before rejection still counts there), a GitHub-style daily-activity calendar, weekly momentum by source vs. your goal, source/work-type breakdowns, a response-rate gauge, and a slowly rotating 3D terrain of weekly volume by status (echarts-gl, with a 2D fallback when WebGL is unavailable). Computed from your data; nothing is estimated. ECharts 5.6.0 + echarts-gl 2.0.9 are vendored in `static/vendor/` so it all works offline.
