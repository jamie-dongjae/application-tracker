...
from tracker.importers.matching import (  # noqa: E402
    V4_FIELDS, build_patch, match_record, new_record,
)
...
# ---------------------------------------------------------------------------
# Matching (engine lives in tracker/importers/matching.py; aliases stay here —
# they name her actual pipeline and must not ship in the repo module)
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    def plan(md: dict, *, stale: bool = False, add_if_missing: bool = True,
        ...
        if md.get("force_add"):
            adds.append(md)
            return
        existing, reason = match_record(md, apps, aliases=COMPANY_ALIASES,
    ...
    for md in LIVE + NON_APPS:
        ...
        if id(md) in added_ids:
            ...
            existing, _ = match_record(md, store.list_applications(), aliases=COMPANY_ALIASES)
